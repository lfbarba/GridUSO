...
Note that $F$ consists of four subgrids being $A\times B, A'\times B, A\times B'$ and $A'\times B'$. Let their respective sinks be $s_{A,B}$, $s_{A',B}$, $s_{A,B'}$ and $s_{A',B'}$.
...
If the $A\times B$-subgrid  is the sink of the $\A$-$\B$-partition grid $H$, then the sink of $A\times B$ is the sink of $G$.
\end{theorem}
\begin{proof}
...
To see this fix some $v_j$ and consider the $(2,2)$-subgrids containing both $v_j$ and $v_i$ for $1\leq i < j$. 
Note that each of these subgrids is a USO and none of them contains a path from $v_i$ to $v_j$ due to the assumption on having a linear extension. Therefore $v_j$ can't be the source of any of the subgrids and in every subgrid $v_j$ has at least one incoming edge. The edges of two different subgrids are disjoint so that there are $j-1$ subgrids each contributing at least one incoming edge for $v_j$. This concludes the proof.
...
Continuing the second phase we first find some $v \in \join(V')$ by using at most $O(V')$ additional queries and note that $v' \succeq v$ for every $v' \in V'$. Let $I'$ be the set of indices containing the first coordinate of each vertex in~$V'$. 
...
Because of the renaming after the first phase and due to Lemma~\ref{lem:diagonal_lemma} we know that $v_i$ has at least $i-1$ incoming edges. Recall the definition of $V'$ in the start of the second phase. 
...
Recall that $v$ is the sink of the $I_v\times J_v$-grid by Observation~\ref{Obs:Sink of dominated grid}, where $|I_v| \geq \frac{m}{2}$ and $|J_v|\geq \frac{n}{2}$. Let $A_1\subset I_v$ and $B_1\subset J_v$ be subsets of indices such that $|A_1| = \frac{m}{2}, |B_1| = \frac{n}{2}$ and note that $v$ is also the sink of the $A_1\times B_1$-subgrid.
