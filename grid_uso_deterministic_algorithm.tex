...
\begin{theorem}\label{theorem:Sink of Induced USO's}
Let $G = K_X \times K_Y$ be an oriented grid,
and let $A$ and $B$ be partitions of $X$ and $Y$, respectively.
...
\begin{lemma}\label{lemma:n/4, n/4 in square grids}
...
\begin{theorem}\label{theorem: n/4 indegree}
Given an $[m]\times[n]$-grid USO $G$, we can find a vertex with \indegree $[a,b]$ in $G$ such that $a \geq \frac{m}{4} - 1$ and  $b \geq \frac{n}{4} - 1$ using $\mathcal{O}(n + m)$ vertex queries.
\end{theorem}

\begin{proof}
Assume without loss of generality that $m \geq n$. Assume for ease of explanation that $m \bmod n = 0$, this assumption will be removed at the end of the proof.
Let $\mathcal A$ be a partition of $[m]$ into $n$ pairwise-disjoint sets, each with $\frac{m}{n}$ indices. 
Let $\mathcal B = [n]$ and consider the grid $H  = K_{\mathcal A}\times K_{\mathcal B}$ and its orientation induced by $G$.
Since $H$ is an $(n, n)$-grid USO, we can use Lemma~\ref{lemma:n/4, n/4 in square grids} to find a vertex $v$ with a \indegree $[c,d]$ in $H$ satisfying $c, d \geq \frac{n}{4} - 1$. This process requires to query $\mathcal{O}(n)$ vertices of $H$. However, to obtain the orientation of the edges adjacent to a vertex $u$ of $H$, we need to first find the sink of $G(u)$.
Recall that $G(u)$ is an $(1, \frac{n}{m})$-grid. Therefore, we can query the sink of $G(u)$ after querying $\frac{n}{m}$ vertex of $G$. Thus, in total to find $v$ we need to perform $O(n\cdot \frac{m}{n}) = O(m)$ vertex queries in $G$.

Assume that $v = (A, B)$, where $A\in \mathcal A$ and $B \in \mathcal B$.
Let $I_v\subseteq \mathcal A$ be the family of sets such that  $I_v \times \{y_v\}$ is the set of all vertices of $H$ with an outgoing horizontal edge to $v$, with $v$ included in this set.
Analogously, $J_v\subseteq \mathcal B$ is the family of sets such that $\{x_v\}\times J_v$ is the set of all vertices of $H$ with an outgoing vertical edge to~$v$ (including $v$). Notice that $|I_v| =c + 1$ while $|J_v| = d + 1$.
Since $v$ has only incoming edges in the $I_v\times J_v$-grid, $v$ is the unique sink of this subgrid of $H$. 

Let $X = \cup I_v$ and let $Y = \cup J_v$. Since each set of $\mathcal A$ contains $\frac{m}{n}$ indices~of~$[m]$, we know that 
$$|X| = |I_v|\cdot \frac{m}{n} = (c+1)\cdot \frac{m}{n} \geq \frac{n}{4} \cdot \frac{m}{n} = \frac{m}{4}.$$
Moreover, $|Y| = |J_v| = d+1 \geq \frac{n}{4}$.
Let $s_v$ be the vertex of $G$ being the sink of $G(v)$. 
Because $v$ is the sink of the $I_v\times J_v$-grid, and since $s_v$ is the sink of $G(v)$, by Theorem~\ref{theorem:Sink of Induced USO's} we know that $s_v$ is the sink of the $X\times Y$-grid. Thus, $v$ has \indegree $[a,b]$ in $G$, where $a = |X|-1 \geq \frac{m}{4}-1$ and $b = |Y|-1 \geq \frac{n}{4}-1$.
\end{proof}
