...
\noindent
 %In this paper, 
 We study unique sink orientations (USO) of so-called grids which are digraphs whose underlining graphs are Cartesian products of complete graphs.
    \MM{``Underlining graph'' -- Jerri, you were drunk. Anyways, we should
    agree on ``a grid'' per se being an \emph{undirected} graph.}
...
\subsubsection{Previous work.}
        \MM{In the caption of fig.~1: We should probably assume a b/w print of
        the proceedings, i.\,e., not refer to PDF colors in the text.}
...
\MM{They are \emph{not} ``introduced'' in this paper.}
...
Assuming general position, the function $f$ induces an orientation on $G$ such that for every oriented
...
This algorithm translates to a randomized algorithm to find the sink of a square Holt-Klee grid USO using $O(\log^2 n)$ vertex queries~\cite{falkthesis}. However, a matching lower bound for this problem remains elusive.


Another connection was discovered by~\citet{grid05} who showed that every Holt-Klee grid USO is induced by a red-blue arrangement of pseudo-lines which, in turn, can be thought as a combinatorial dual of the one line and $n$ points configuration.
...
While there exist deterministic algorithms with matching performance to solve LP-type problems \cite{chan16,ChazelleM96}, they make an extra assumption  
...
The set system $([m]\times [n], \mathcal R)$ must have constant VC-dimension for the algorithm of \citet{ChazelleM96} to work, where $\mathcal R = \{R_v : v\text{ is a vertex of the grid}\}$. 
Since this set system has unbounded VC-dimension for general Gird USOs as shown in Figure~\ref{}, the above algorithm cannot be applied.
To find the sink of Holt-Klee grids however, the VC-dimension of the aforementioned set system is constant  which leads to deterministic linear-time algorithms~\cite{grid05,falkthesis}.

\AT{Here we refer to the construction which will be in the appendix.}
When this assumption is not satisfied then the above deterministic algorithms are less efficient that exploring the entire 
grid exhaustively. 

Therefore, it is open whether there exist deterministic algorithms that find the sink of a grid USO using only a linear number of queries. In particular, 
it is not known if \emph{any} LP-type problem can be deterministically solved in linear~time. 
...
Another useful property of grids is that to test whether an orientation is a unique sink orientation it enough to only look at small subgrids and ensure acyclicity. The following proposition is essentially known, but not explicitly mentioned before. \JN{At least I haven't seen it naywhere, but I'm not 100\% sure if it is mentioned or not.}
...
For the design of our algorithms to find the sinks of grids, we want to partition the grid into subgrids.
...
Recall the definition of $V'$ in the start of the second phase. 
By the claim that we established, each vertex of $V'$ has at least $\lceil \frac{n}{2} \rceil - 1$ incoming edges.
