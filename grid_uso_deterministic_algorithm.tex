...
We write $\timeEdge(N)$ for the required number of edge queries, $N = m+n$.

By an adversary argument similar to the one used for Theorem~\ref{thm:lowerbound},
we have an $\Omega(N)$ lower bound for $\timeEdge$.
A nearly trivial upper bound is $O(N^2)$; but we can improve on that:
