...
These digraphs are called unique sink orientations of a grid (or simply grid USOs). 
...
All the algorithms mentioned so far are randomized. 
Towards deterministic algorithms, \citet{grid08} hint that an $(m,n)$-grid USO can be naturally written as an LP-type problem of combinatorial dimension 2 (see~\cite{DBLP:journals/algorithmica/MatousekSW96} for the definition of an LP-type problem). 
Randomized algorithms for LP-type problems also imply expected $O(m+n)$ edge queries for grid USOs. There are deterministic algorithms with matching performance \cite{DBLP:journals/jal/ChazelleM96}, but they make an extra assumption which does not hold for grid USOs (see Computational assumption 2 in \cite{DBLP:journals/jal/ChazelleM96}).
...
For $v,w \in G$ we also define an operation called \emph{\join} so that \join($v,w$) is the vertex that is the sink in the smallest subgrid containing $v$ and $w$. Note that $v,w \succeq \join(v,w)$. For sets of vertices $V$, $\join(V)$ denotes the set of vertices of the grid to which there is a path from all vertices in $V$. More formally $\join(V) = \{w \in G \: | \: v \succeq w \:\: \forall v \in V \}$. Given some set of vertices $V$ we can compute some $w \in \join(V)$ with $\mathcal{O}(|V|)$ vertex queries: just repeatedly apply the $\join$ operation on vertices from $V$.
...
The algorithm works as follows. First, query the diagonal vertices $D = \{v_1,\ldots, v_n\}$ where $v_i = (i,i)$.
If one of the vertices in $D$ has the required \indegree, we are done. 
If not, observe that we can assume (by sorting and renaming) that vertex $v_i$ has at least $i-1$ incoming edges. \LB{Don't see this}
...
Let $s = join(\{v_0, \ldots, v_k\})$ and notice that $s$ is smaller than $v_i$ for each $1\leq i\leq k$. Since $k = O(1)$, $s$ can be computed using $O(1)$ vertex queries. 
