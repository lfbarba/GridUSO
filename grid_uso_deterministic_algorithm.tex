...
\begin{proof}
    ...
    Given the grid $G = K_m \times K_n$, let $A \subseteq 2 ^ {[m]}$ and
    $B \subseteq 2 ^ {[n]}$ be partitions of the index sets such that
    $|A| = |B| = 4$, $|a| \le \frac{m}{4}+1$ for all $a \in A$, and $|b| \le
    \frac{n}{4} + 1$ for all $b \in B$.

    Consider the induced unique sink orientation on the grid
    $H = K_A \times K_B$, and let $v$ be a vertex of $H$.
    ...
    $G(v)$, which explains the term $T \left( \frac{n}{4} \right)$.
