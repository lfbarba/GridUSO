...
The Holt-Klee condition is important, as it is the only known simple combinatorial condition that can distinguish geometric from abstract induced grid USOs.
...
In addition, they generalize Random Edge algorithm and prove that it also finds the sink of any Holt-Klee grid USO using $O(\log n\cdot\log m)$ pivot steps with any starting vertex. Finally, they argue that there are $2^{\Omega(n^2)}$ different Holt-Klee grid USOs.
...
Therefore, the randomized algorithm from G\"artner, Morris and R\"ust~\cite{grid08} solves the matrix USO optimization problem on a $m\times n$ matrix in $\Theta(m+n)$ expected time.
Similar optimization problems in matrices have been studied with stronger conditions and have several applications in geometric problems~\cite{aggarwal1987geometric,demaine2005optimizing,galil1992dynamic,mityagin2003complexity}. 
