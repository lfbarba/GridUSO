...
\begin{abstract}
    \noindent
    We study unique sink orientations of grids, which for us are Cartesian products of two complete graphs. 
    Our main contributions are a deterministic algorithm which finds the sink in
    two-dimensional grid USOs using $O(N \log N)$ vertex queries and a deterministic algorithm using $O(N^{\log_4 7})$ edge queries for an $N \times N$ grid. 
    Unique 
\end{abstract}
...
With the above remark, proving this claim establishes the result.
