...
\end{lemma}
\begin{proof}
Consider the $I\cup I'\times J\cup J'$-grid and the partitions $\mathcal A = \{I, I'\}$ and $\mathcal B = \{J, J'\}$ of its sets of indices. Since the gird $H = G_{\mathcal A}\times G_{\mathcal B}$ has an induced USO from $G$, it has a unique sink. Since $u$ is the sink of $G(I, J)$ and is smaller than $v$, $(I,J)$ cannot be the sink of $H$.
\end{proof}
