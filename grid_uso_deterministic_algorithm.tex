...
A \emph{grid} is the Cartesian product $K_m \times K_n$ (also known as an $(m, n)$-grid) of two complete graphs with $m$ and $n$ vertices, respectively. An induced subgraph of a grid is called a \emph{subgrid} if it is itself a grid.
...
In these two models, we want to determine the minimum number of oracle queries that an algorithm needs to ask to find the sink of a grid USO in the worst-case.
...
Since this polytope is a product of simplices, its graph $G$ is isomorphic to a grid~\cite{grid05}. 
...
It is know that the above problem is equivalent to the \emph{One line and $n$ points} problem in the plane~\cite{linepoint,welzl2001entering}:
...
We assume that $\ell$ contains non of the given points and that no two segments intersect on $\ell$.
G\"artner et al.~\cite{linepoint} proved that every instance of the one line and $n$ points problem gives rise to special grid orientations, called \emph{Holt-Klee} grid USOs, which are grid USOs with a specific forbidden minor called the ``double twist''.
In addition, they design an instance of a Holt-Klee grid USO that does not come from a one line and $n$ points instance. 
The authors interpret the \emph{Random Edge} simplex algorithm in the setting of one line and $n$ points and prove that it finds the lower-most segment using $O(\log^2 n )$ pivot steps. 
They also provide an instance where this algorithm needs $\Omega(\log^2 n )$ many pivot steps. 
This algorithm translates to a randomized algorithm to find the sink of a square Holt-Klee grid USO using $O(\log^2 n)$ vertex queries~\cite{falkthesis}. However, no matching lower bound is known for this problem.

Another connection was discovered by G\"artner et al.~\cite{grid05} who showed that every gird HK is induced by a red-blue arrangement of pseudo-lines which, in turn, can be thought as a dual of the one line and $n$ points configuration. 
In addition, they generalize the result of G\"artner et al.~\cite{linepoint} to conclude that the Random Edge algorithm needs $O(\log n\cdot\log m)$ pivot steps to find the sink of any Holt-Klee grid USO with any starting vertex. 
...
Therefore, the algorithm from G\"artner, Morris and R\"ust~\cite{grid08} solves the matrix USO optimization problem on a $m\times n$ matrix in $\Theta(m+n)$ time. 
Similar optimization problems in matrices have been studied with stronger conditions and have several applications in geometric problems~\cite{aggarwal1987geometric,demaine2005optimizing}. 
While many of these variants can be solved using efficient deterministic algorithm, no non-trivial deterministic algorithm for the matrix USO optimization problem was known prior to this work.
...
However, this bound is far from the best $\Omega(m+n)$ lower bound known for this problem. 
Since an algorithm that finds the sink of a grid USO in the edge query model can be used to solve the matrix USO optimization problem, 
our result provides a deterministic algorithm that finds the minimum of an $m\times n$ matrix USO by querying for the value of the function in  only $O((m+n)^{\log_4 7})$ entries of the matrix. 

Since a single vertex query reveals the orientation of a linear number of edges, one would expect the number of required queries to be different in the edge and the vertex query models.
Indeed, as shown by G\"artner et al.~\cite{grid08}, there is an exponential gap between the number of queries required by these two models.
