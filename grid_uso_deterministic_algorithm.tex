...
This algorithm translates to a randomized algorithm to find the sink of a square Holt-Klee grid USO using $O(\log^2 n)$ vertex queries~\cite{falkthesis}. 
In addition, the Random Edge algorithm is generalized by \citet{grid05}, where they prove that it also finds the sink of any Holt-Klee $(m,n)$-grid USO using $O(\log n\cdot\log m)$ pivot steps with any starting vertex. 
