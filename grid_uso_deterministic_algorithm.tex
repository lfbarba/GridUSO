...
\begin{corollary}\label{corollary: n/4 indegree}
  Given an $[m]\times [n]$ grid USO $G$, we can find a vertex with \indegree $[a,b]$ with $a \geq \frac{m}{8} - 1$ and  $b \geq \frac{n}{8} - 1$ using $O(nM)$ vertex queries where $M = \max\{m,n\}$.
\end{corollary}
...
consider the $\A$-$\B$-partition grid $H = K_\A\times K_\B$ and its induced orientation inherited from $G$. 
 ...
 The total number of vertex queries of $G$ used is therefore $O(n\frac{m}{n}\cdot n) = O(nm)$ as asserted. 
