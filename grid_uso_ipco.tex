...
\title{Deterministic Algorithms for Unique Sink Orientations of Grids}

% a short form should be given in case it is too long for the running head
\titlerunning{Deterministic Algorithms for USOs of Grids}
...
\keywords{Unique sink orientation, LP-type problem, Violator spaces}
\end{abstract}
...
See Fig.~\ref{fig:examplegrid} for an example of a grid.
...
We denote by \emph{\sinkmn} the \sink problem where the input is an $(m,n)$-grid.
The goal of this paper is to provide bounds on the minimum number of queries that a deterministic algorithm must ask in the worst case to solve \sinkmn.
...
\vspace{-.2in}
  \begin{figure}[h] 
  	...
  	\caption{\small Left: Example of a $(4,3)$-grid (that means $4$ columns and $3$ rows) 
  	where a $(3,2)$-subgrid is shaded. Right: A unique sink orientation of a $(3,2)$-grid. It also represents the double twist graph forbidden as a subgraph in Holt-Klee grid USOs.} 
...
%Therefore, minimizing~$f$ on the polytope corresponds to finding the unique sink of the grid USO $G$.
...
\citet{linepoint} proved that the grid USO arising from an instance of the one line and $n$ points problem is \emph{Holt-Klee}, i.e., a grid USO with a specific forbidden subgraph called the ``double twist'' (depicted in Fig.~\ref{fig:examplegrid} right).
%In other words, each instance of the one line and $n$ points, and hence also each instance of a linear program on a $(d, d+2)$-polytope, corresponds to a Holt-Klee grid USO. 
...
For general grid USOs, \citet{grid08} exhibit an algorithm that solves \sink on an $(m,n)$-grid USO using $O(\log n\cdot  \log m)$ vertex queries.
The best lower bound for this problem is $\Omega(\log n + \log m)$ (obtained from finding the sink of a single row or column).
In the edge query model, they show that \sink can be solved using $\Theta(m+n)$ edge queries in expectation. 
...
%In particular, it finds the sink of any 2-dimensional grid USO using expected $O(\log n \cdot \log m)$ vertex queries which matches the performance of Random Edge for Holt-Klee grid USOs. 
%Note, however, that a matching lower bound for the problem in the randomized setting remains elusive.
...
%%maybe we can reduce
Consider the following \emph{matrix USO optimization} problem: Find the global minimum of an injective real function defined on a two dimensional matrix USO, where a matrix is USO if each $2\times 2$ sub-matrix has a unique local minimum. 
...
\vspace{-.2in}\subsubsection{Connection to LP-type problems.} \citet{grid08} hinted that \sink
%the problem of finding the sink of an $(m,n)$-grid USO 
...
To write \sink as an LP-type problem, we let the set of constraints $S$ be the set of rows and columns and thus $|S| = m + n$. As any grid USO is acyclic (see Section~\ref{section:grid_uso_properties}), there is a topological ordering of its vertices with the sink having the highest rank in the ordering. 
...
For grid USOs these operations corresponds directly to edge queries, which by the above construction yield randomized algorithms that use a linear number of edge queries to solve \sink. 

While derandomizations of algorithms for LP-type problems exist with the same performance~\cite{chan16,ChazelleM96}, they make an extra assumption on the problem that does not hold for grid USOs, namely the existence of a \emph{subsystem oracle} (see Computational assumption 2 in \cite{ChazelleM96}).
Other problems where these assumptions do not hold have been studied~\cite{chan1998deterministic}, and it is an open question whether or not all of them can be solved with a linear number of primitive operations.
...
Intuitively, instead of assigning a value to each subset $A\subseteq S$ of constraints, in violator spaces we map $A$ to a set of its \emph{violators}, i.e., the set of constraints whose addition to $A$ would change its basis. The goal is to find the basis with the same set of violators as $S$ which can be done in linear expected number of primitive operations~\cite{DBLP:journals/dam/GartnerMRS08}.

When modeling grid USOs as violator spaces, we assign to each vertex the subset of constraints to which it has an outgoing edge.
...
%Since this set system has unbounded VC-dimension for general grid USOs, as illustrated in Fig.~\ref{fig:shatteredgrid}, these deterministic algorithms are less efficient than exploring the entire grid exhaustively. 
...
We present two deterministic algorithms to \sinkmn using $O((m+n)\log (m+n))$ vertex queries
and $O((m+n)^{\log_47})$ edge queries, respectively. 
Additionally, if the Holt-Klee property holds (see Fig.~\ref{fig:examplegrid} right), we exhibit a deterministic algorithm to find the sink using $O(m + n)$ vertex queries. 
Both query models exhibit a lower bound of $\Omega(m + n)$.
...
In Section~\ref{section:grid_uso_properties} we state basic facts for grid USOs that were already known from previous work. Subsequently, we define a way to partition the grid into subgrids and discuss the properties of this partition. 
...
which inherits a unique sink orientation from the original grid USO; see Fig.~\ref{fig:example_induced_orientation} for an illustration.
  \begin{figure}[htbp] 
  	...
  	\caption{\small Left: A $(3,3)$-grid USO whose %$X$-coordinates and $Y$-
...
%find the sink of an $(m, n)$-grid USO.
...
Because grid USOs are acyclic~\citet{grid08}, this partial order is well defined.
...
Consider the following algorithmic approach to solving \sinkmn
%finding the sink of an ($m$,$n$)-grid USO deterministically. 
...
By Lemma~\ref{lemma:USO-Lemma} we could thereafter discard one subgrid out of consideration and possibly still find the sink of one more subgrid.
...
%to find the sink of an $(m,n)$-grid,
...
%\end{figure}
 ...
 %If we can show that there is an edge from $w$ to $v^*$, then by acyclicity $v^*$ has at least one more incoming vertical edge as compared to $w$, therefore establishing that $b \geq \frac{\lceil \frac{n}{2}\rceil-1}{2} + 1 \geq \frac{n}{4} \geq \frac{n}{4} - 1$ and showing that$v^*$ is an $(\frac{n}{4}, \frac{n}{4})$-vertex. 
  ...
  %We give illustrations of the two cases in Fig.~\ref{fig:seedlem2}. 
...
\begin{figure}[h]
\centering
\includegraphics[width=0.90\textwidth]{ClimbingLemma.pdf}
...
\centering
\hspace*{0.75cm}
\includegraphics[width=0.83\textwidth]{ClimbingLemma-2.pdf}
...
From Lemma~\ref{lemma:Climbing lemma} we immediately get the following corollary.
\begin{corollary}[$*$]\label{corollary: (m/2,n/2) indegree}
Let $G$ be an $(m,n)$-grid USO. 
We can compute an $( \frac{m}{2}, \frac{n}{2})$-vertex using $O(m + n)$ vertex queries.
\end{corollary}

Our main result can now be proved from Corollary~\ref{corollary: (m/2,n/2) indegree}.
...
A grid USO is called \emph{Holt-Klee} if it has a specific forbidden subgraph called the ``double twist'' (depicted in Fig.~\ref{fig:examplegrid} right).
...
\end{lemma}
\begin{proof}
Let $(v_1, v_2, v_3,v_4)$ be directed path in the bow $H$ and assume that the edge $v_2v_3$ is horizontal. 
...
either (1) $u$ is the sink of $G$, or (2)
 $u$ has no horizontal incoming edges, in which case the sink of $G$ cannot lie in the row containing $u$, or (3) there is an outgoing edge from $u$ to some vertex $a$. 
...
\section{Conclusions}
In this paper, we have discussed the existence of efficient deterministic algorithms to find the sink of an $(m,n)$-grid. 
We show that $O((m+n)\log(m+n))$ vertex queries and $O((m+n)^{1.404})$ edge queries suffice to find this sink. In the case of Holt-Klee grids, a linear number of vertex queries is suffices.
%Recall that a  sink-algorithm in the edge query model can be used to solve the matrix USO optimization problem. 
%Therefore, our sink-algorithm in the edge query model provides a way to deterministically find the minimum of an $m\times n$ matrix USO by querying for the value of only $O((m+n)^{1.404})$ entries of this matrix. 

While linear lower bounds are simple to obtain, closing the gap between the lower and upper bounds remains an open question. 
...
Reaching a linear bound seems challenging, but new techniques for computing $\epsilon$-nets without the need of a subsystem oracle could lead to optimal algorithms. 

\section*{Acknowledgments}\label{sec:Acknowledgments}

This work started at the 13th Gremo’s Workshop on Open Problems
(GWOP), June 1–5, 2015, Feldis (GR), Switzerland. 
The authors would like to thank Bernd G\"{a}rtner and Emo Welzl for the workshop and for useful discussions.
...
\newpage
\appendix
%\section{Subsystem oracle}
%
%For each vertex $v$ in the grid, let $R_v\subset S$ be the set of rows and columns to which $v$ has an outgoing edge. 
%The \emph{violator set system} is the pair $(S, \mathcal R)$, where $\mathcal R = \{R_v : v\text{ is a vertex of the grid}\}$. 
%
%A necessary requirement to have a subsystem oracle as defined by~\citet{ChazelleM96} is that $(S, \mathcal R)$ has constant VC-dimension (refer to~\cite{matouvsek2002lectures} for further information on set systems and VC-dimension). 
%Since this set system has unbounded VC-dimension for general grid USOs, as illustrated in Fig.~\ref{fig:shatteredgrid}, no subsystem oracle exists.
%
%
%%
%%Curiously, if the grid USO satisfies the Holt-Klee condition, then its violator set system has VC-dimension at most 2.
...
%For any $A, A'\in \A$ and $B,B'\in \B$, let $F$ be the $\{A,A'\}\times\{B, B'\}$-subgrid of~$H$.
...
%It is enough to check that the orientation of $F$ is a unique sink orientation and that $H$ is acyclic.
