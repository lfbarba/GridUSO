...
This equivalence implies that the violator set system for Holt-Klee grid USOs, as we defined it above, has constant VC dimension. 
...
\JN{I still need to verify this.}
...
\end{lemma}
\AT{Lemma now contains what was Theorem 1. Please verify this phrasing is OK.}
%\begin{theorem}
...
Consider the following algorithmic approach to finding the sink of an ($n$,$n$)-grid USO deterministically. \AT{Decide when to talk about square grids. Also see Outline subsec.}
...
To proof of Theorem~\ref{thm:timeEdge} builds on the following result for the \emph{vertex oracle model}.
...
\begin{proof}[of Theorem~\ref{thm:timeEdge}]
    ...
    Let $v$ be a vertex of $H$. Note that $v$ is identified with an $(\frac{m}{4},\frac{m}{4})$-subgrid of $G$.
    We can find the sink $s$ of the subgrid $v$ using a recursive call that takes $T(N/4)$ edge queries.
    Afterwards we can find out the orientations of the six $H$-edges
    adjacent to the vertex $v$ by querying the $O(N)$ many $G$-edges adjacent to $s$.
    Thus we have simulated a vertex query in $H$ using $T(N/4) + O(N)$ edge queries in $G$.
    ...
    \]
    edge queries. Recall that the algorithm from Lemma~\ref{lem:4:7} also queries the sink $s_H$ of $H$, and by Lemma~\ref{lemma:USO-Lemma} the sink $s_G$ of $G$ equals the sink of the subgrid $s_H$.
    Therefore we have also found the global sink.
    \qed
\end{proof}
