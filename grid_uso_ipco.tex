...
In this case, since $u$ is the sink of its row, the (2,2)-subgrid containing $u,v$ and $a$ is necessarily a bow and hence, the sink of $G$ cannot lie in the column containing~$u$; see Fig.~\ref{fig:Bow example}.
Thus, regardless of the case, we can discard either a row or a column of $G$ and update either $u$ or $v$ to continue with this process.
Consequently, after querying all vertices in the initial row $r$, with every additional query we can discard either a row or a column. Thus, after $O(n+ m)$ vertex queries the algorithm finds the sink of $G$.
\qed
\end{proof}
