...
\titlerunning{Grid USO Algorithms}
...
\noindent\LB{There was no mention of $n$, I included it, although it might be misleading in thinking that it is a square grid. But since it is not less general, I figure it is not so bad}
 We study unique sink orientations (USO) of grids --- Cartesian products of \emph{two} complete graphs with $n$ vertices whose edges are oriented in such a way that each subgrid has a unique sink.
...
\section{Conclusions}
In this paper, we discuss the existence of efficient deterministic algorithms to find the sink of a $(m,n)$-grid. In the vertex oracle model 
we prove that at most $O((m+n)\log(m+n))$ queries are enough. In the edge oracle model we prove that at most $O((m+n)^{1.4})$ queries are enough.

Note that for both oracle models linear lower bounds are simple to obtain, but nothing better is known. Thus, the most natural open problem
