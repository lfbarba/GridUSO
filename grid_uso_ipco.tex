...
\and
Department of Computer Science, ETH Z{\"u}rich, Switzerland  \\
%CH-8092 Z{\"u}rich, Switzerland, 
{\tt \{mmilatz, njerri, athomas\}@inf.ethz.ch}
}
...
The goal of this paper is to provide bounds on the minimum number of queries that a deterministic algorithm must ask in the worst case to solve \sinkmn.
  ...
  \begin{figure}[h] 
  	...
  	\caption{\small Left: Example of a $(4,3)$-grid (that means $4$ columns and $3$ rows) where a $(3,2)$-subgrid is shaded. Right: A unique sink orientation of a $(3,2)$-grid. It also represents the double twist graph forbidden as a subgraph in Holt-Klee grid USOs.} 
...
For general grid USOs, \citet{grid08} exhibit an algorithm that solves \sinkmn using $O(\log n\cdot  \log m)$ vertex queries.
...
For grid USOs these operations corresponds directly to edge queries, which by the above construction yield randomized algorithms that use a linear number of edge queries to solve \sinkmn. 
...
In this paper, we have discussed the existence of efficient deterministic algorithms to solve \sinkmn. 
