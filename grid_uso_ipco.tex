...
\newcommand{\appen}{\ensuremath{\textsc{Appendix}}}
...
on an $(m,n)$-grid USO.
%where the input is an $(m,n)$-grid. \AT{The input is not a grid, is it?}
...
%that a deterministic algorithm must ask in the worst case to solve \sinkmn.
...
Analyzing \emph{Random Edge} simplex algorithm for Holt-Klee grid USOs yields an algorithm to solve \sink that queries $\Theta(\log n \cdot \log m)$ vertices in expectation~\cite{linepoint,grid05,falkthesis}.
...
The matrix is given by an oracle that reveals the value of the queried entry.
Since this matrix yields a grid USO when edges between entries in the same row or column are oriented from the larger to the smaller, the randomized algorithm from~\citet{grid08} solves the matrix USO optimization problem on a $m\times n$ matrix by querying $\Theta(m+n)$ entries in expectation. However, no deterministic counterpart to this algorithm is known. 
...
hence, by the above construction we can solve \sinkmn with the randomized algorithms of \cite{MatousekSW96} using a linear number of edge queries.
...
%all of them can be solved with a linear number of primitive operations.
...
Intuitively, instead of assigning a value to each set $A\subseteq S$ of constraints, %in violator spaces 
...
to the best of our knowledge no non-trivial deterministic algorithm was known prior to this work. This is mainly due to the fact that the aforementioned subsystem oracle does not exist. 
%We include a detailed discussion about the non-existence of the subsystem oracle in Appendix~\ref{app:nosubsystem}.
...
Additionally, if the Holt-Klee property holds, %(see Fig.~\ref{fig:examplegrid} right), 
...
%A proof for the lower bound can be found in Appendix~\ref{app:determlowerbound}.
...
In Section~\ref{section:grid_uso_properties} we state basic facts for grid USOs %that were already known from previous work. 
and define a way to partition the grid into subgrids. % and discuss the properties of this partition. 
...
%Due to space constraints, the proofs of some results marked with $(\appen)$ are omitted and can be found in Appendix~\ref{app:missingproofs}.
...
\begin{lemma}[\appen]\label{lemma:USO-Lemma}
...
\begin{corollary}[\appen]\label{corollary: n/4 indegree}
...
\begin{corollary}[\appen]\label{corollary: (m/2,n/2) indegree}
Let $G$ be an $(m,n)$-grid USO. 
...
\begin{theorem}[\appen]\label{theorem:Sink algorithm}
...
%
%In this section we provide evidence that the knwon deterministic algorithms to solve LP-type problems \cite{chan16,ChazelleM96} will fail 
%to solve \sinkmn faster than the algorithms presended in this paper. 
%As we have already mentioned, these algorithms depend
...
%The \emph{VC-dimension} of $(S, \mathcal R)$ is defined as the size of the largest shatter subset of $S$. 
%%The shatter function $\pi_{\mathcal R}$ gives the maximum possible number of subsets 
%
...
%
%Firstly, we can argue that for general grid USOs the set system $(S, \mathcal R)$ that we define above  
%has VC-dimension $\Omega(\log n)$. We prove this with a counterexample in Fig.~\ref{fig:shatteredgrid}. 
%
%
%\begin{figure}[t] 
%  	\centering
...
%%
%%However, the requirement for a subsystem oracle is that it computes $\mathcal R|_A$ in $O(|A|)^{d+1}$ time.
...
%
%Therefore, we conclude that the subsystem oracle does not exist in our case. It follows that the deterministic algorithms 
%for solving LP-type problems are less efficient than exploring the entire grid exhaustively.  It is, thus, an open problem if \emph{any} LP-type problem can be deterministically solved in linear~time. 
