...
\citet{linepoint} proved that the grid USO arising from an instance of the one line and $n$ points problem has a specific forbidden subgraph called the ``double twist'' (depicted in Fig.~\ref{fig:examplegrid} right) and such grids are called \emph{Holt-Klee}. 
...
Analyzing \emph{Random Edge} simplex algorithm for Holt-Klee grid USOs yields an algorithm that queries $\Theta(\log^2 n)$ vertices in expectation~\cite{linepoint,grid05,falkthesis}.
For general grid USOs, \citet{grid08} exhibit an algorithm that finds the sink of an $(m,n)$-grid USO using $O(\log n\cdot  \log m)$ vertex queries.
The best lower bound for this problem is $\Omega(\log n + \log m)$ (obtained from finding the sink of a single row or column).
In the edge query model, they provide an optimal randomized algorithm to find the sink using $\Theta(m+n)$ edge queries in expectation. 
...
For grid USOs these operations corresponds directly to edge queries, which by the above construction yield randomized algorithms that use a linear number of edge queries to find the sink. 
...
%Therefore, our sink-algorithm in the edge query model provides a way to deterministically find the minimum of an $m\times n$ matrix USO by querying for the value of only $O((m+n)^{\log_4 7})$ entries of this matrix. 
...
Due to space constraints, some proofs are omitted and can be found in the appendix.
...
Holt-Klee grid USOs arise naturally as a generalization of the one line and $n$ points problem~\cite{linepoint}.
Recall that a Holt-Klee grid USO are those USOs with a specific forbidden subgraph called the ``double twist'' (depicted in Fig.~\ref{fig:examplegrid} right).
...
\end{lemma}
\begin{proof}
Let $v_1, v_2, v_3$ and $v_4$ be directed path in the bow $H$ and assume that the edge $v_2v_3$ is horizontal. 
Let $w$ be any vertex in the row containing $v_2v_3$.  
Assume for a contradiction that $w$ is the sink of $G$. 
Since $w$ has all its edges incoming, the orientation of all the edges of the $(3,2)$-subgrid containing $H$ and $w$ is fixed as shown in Fig.~\ref{fig:Bow example}. This yields a double twist as a subgraph of $G$ which is a contradiction. Thus, no vertex in the row containing $v_2v_3$ is the sink of~$G$. \qed
\end{proof}
...
Thus, regardless of the case, we can discard either a row or a column of $G$ and update either $u$ or $v$ accordingly to continue with this process.
...
\begin{theorem}
    \label{thm:timeEdge}
    There exists a deterministic algorithm which finds the sink of an $(m,n)$-grid USO using $O((m+n) ^ {\log_4(7)})$ edge queries, where $\log_4(7) < 
    1.404$.
\end{theorem}
...
\vspace{-0.1in}
\section{Conclusions}
In this paper, we have discussed the existence of efficient deterministic algorithms to find the sink of a $(m,n)$-grid. 
We show that $O((m+n)\log(m+n))$ vertex queries and $O((m+n)^{1.4})$ edge queries suffice to find this sink. In the case of Holt-Klee grids, a linear number of vertex queries is suffices.
