...
\vspace{-.2in}
  \begin{figure}[htbp] 
  	...
  	and a $(3,2)$-subgrid is shaded. Right: The orientation is a unique sink orientation of a $(3,2)$-grid. It also represents the double twist graph forbidden as a subgraph in Holt-Klee grid USOs.} 
...
(for more information on LP-type problems refer to \cite{MatousekSW96}). To solve an LP-type problem, one needs to find a basis with the same value under $w$ as the whole set $S$.
...
The function $w$ can be defined as follows. Every subset of the rows and columns defines a subgrid and this subgrid is mapped to the rank of its sink in the topological ordering. 
If this subset of $S$ consists only of rows or only of columns, its value under $w$ is~$-\infty$. 
...
While derandomizations of algorithms  algorithms exist with the same performance~\cite{chan16,ChazelleM96}, they make an extra assumption on the problem that does not hold for grid USOs, namely the existence of a \emph{subsystem oracle} (see Computational assumption 2 in \cite{ChazelleM96}).
...
\subsection{Holt-Klee grid USOs}

Holt-Klee grid USOs arise naturally as a generalization of the one line and $n$ points problem~\cite{linepoint}.
Recall that a Holt-Klee grid USO has a specific forbidden subgraph called the ``double twist'' (depicted in Fig.~\ref{fig:examplegrid} right).
...
The authors would like to thank Bernd G\"{a}rtner and Emo Welzl for useful discussions.
