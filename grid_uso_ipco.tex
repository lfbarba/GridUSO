...
The goal of this paper is to provide bounds on the minimum number of queries that a deterministic algorithm must ask in the worst case to solve \sink.
...
We believe that finding the sink in grid USOs is a good candidate for an LP-type problem where super linear lower bounds might exist. 
...
We present two deterministic algorithms to find the sink of an $(m,n)$-grid USO using $O((m+n)\log (m+n))$ vertex queries
and $O((m+n)^{\log_47})$ edge queries, respectively. 
Additionally, if the Holt-Klee property holds (see Fig.~\ref{fig:examplegrid} right), we exhibit a deterministic algorithm to find its sink using $O(m + n)$ vertex queries. 
