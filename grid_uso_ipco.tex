...
The main goal is to determine the minimum number of queries that a deterministic (or randomized) algorithm must ask in the worst case to find the sink of a grid USO. We call such an algorithm a \emph{\sinkalg}. \AT{Why do we need to speak of \sinkalg ?}
...
Assuming general position, \AT{Assuming general position of what?} $f$ induces an orientation on $G$ such that for every oriented
...
It is known that the above\AT{By above here we mean the d,d+2 polytope problem?} problem is equivalent to the \emph{One line and $n$ points} problem in the plane~\cite{linepoint,welzl2001entering}:
...
It can also be shown that geometry is not essential for Random Edge: Finding the sink of a Holt-Klee $(m,n)$-grid USOs it takes also $O(\log n\cdot\log m)$ vertex queries~\cite{grid05,falkthesis}. However, a matching lower bound for this problem remains elusive. \AT{We have to stress out that this sentence refers to the randomized algorithmic setting}
...
\citet{grid08} study higher dimensional grid USOs (Cartesian products of $d$ complete graphs, each on at most $n$ vertices) as general models that arise from solving a problem called PGLCP. \AT{We either say PGLCP is or we don't write PGLCP at all.} 
...
It is known that every Holt-Klee grid USO is induced by a red-blue arrangement of pseudo-lines \cite{grid05,falkthesis}. \AT{Do we really need to say that it can be thought of as... ?}
...
That is, we obtain a deterministic algorithm that finds the sink of an $(n, n)$-grid using $O(n \log n)$ vertex queries. \AT{I leave the outline section unedited. Why does it speak only of square grids?}
...
\end{lemma}
\AT{Should we merge lemma and theorem into one?}
\begin{theorem}
...
Consider the following algorithmic approach to finding the sink of an ($n$,$n$)-grid USO deterministically. \AT{Again, why talk about square grids here?}
...
The next step would be to find the sink of the block antipodal to the block containing $v$ in the (2,2)-grid induced by this partition. \AT{I chaned oposite to antipodal which is standard terminology in the cubes (and 2x2 is also a cube)}
...
\end{align*}
where $T(n)$ is the number of vertex queries needed by this algorithm to find the sink of an $(n,n)$-grid. This recurrence solves to $T(n) = O(n\log n)$. 
...
Rename the coordinates such that for all $1 \leq i < j\leq n$ either $v_i \succeq v_j$ or $v_i$ and $v_j$ are incomparable. \AT{We have not define incomparable now. Also bellow we cant say since $v_i \succeq v_j$; they might also be incomparable}
...
\begin{corollary}\label{corollary: n/4 indegree}
  Given an $(m,n)$-grid USO $G$, we can find a vertex with \indegree $[a,b]$ with $a \geq \frac{m}{8} - 1$ and  $b \geq \frac{n}{8} - 1$ using $O(M)$ vertex queries where $M = \max\{m,n\}$.
\end{corollary} \AT{Changed $[m]\times [n]$ to $(m,n)$. Here and below.}
...
\begin{theorem}\label{theorem:Sink algorithm}
 Let $M = \max\{m,n\}$. The sink of an $(m,n)$-grid USO can be found after $O(M(\log M))$ vertex queries.
\end{theorem}
...
\begin{theorem}
    \label{thm:timeEdge}
    To find the sink in a grid USO deterministically, \AT{We have to define $N$}
...
\begin{proof}
    ...
    $m=n$ (otherwise we can use an embedding argument). \AT{Is "embedding argument" clear enough? Why do we need m power of 4? I guess we need to insert the 7 steps proposition!}
    ...
    $G(v)$, which explains the term $T(N/4)$. \AT{$G(v)$ has not been defined}
