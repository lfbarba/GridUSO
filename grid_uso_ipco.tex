...
\titlerunning{Deterministic Algorithms for USOs of the Grid}
...
\citet{linepoint} proved that the grid USO arising from an instance of the one line and $n$ points problem is a special grid orientation, called \emph{Holt-Klee} grid USO, which is a grid USO with a specific forbidden subgraph called the ``double twist'' depicted in Fig.~\ref{fig:examplegrid} (right). In other words, each instance of the one line and $n$ points, and hence also each instance of a linear program on a $(d, d+2)$-polytope, corresponds to a Holt-Klee grid USO. 
...
In Section~\ref{section:the_sink_finding_algorithm} we focus on the vertex query model and prove a our main result: Theorem~\ref{theorem:Sink algorithm}. Finally, in Section~\ref{section:The edge query model} we briefly discuss the edge query model and give the relevant algorithm in Theorem~\ref{thm:timeEdge}. We state some proofs in full detail; those omitted can be found in the appendix.
...
\vspace{-0.1in}
\section{Conclusions}
In this paper, we have discussed the existence of efficient deterministic algorithms to find the sink of a $(m,n)$-grid. In the vertex query model 
we proved that at most $O((m+n)\log(m+n))$ queries are enough. In the edge query model we proved that at most $O((m+n)^{1.4})$ queries are enough.
...
%Or this method might give linear upper bounds for more restricted classes of grid USO. 

Finally, it still might be possible 
to solve Holt-Klee grid USOs in deterministic linear time by using the algorithms mentioned in Section~\ref{sec:intro}.
Even though the obvious approach does not work, there may be another way to make use of these algorithms for our purpose.
